...
#region Private Helper Functions
        ...
        {
            HTTPManager.Logger.Verbose(nameof(EventSource), $"CallOnError({this.State}, {error}, {msg})", this.LoggingContext);
        ...
        {
            HTTPManager.Logger.Information("EventSource", $"SetClosed({this.State}, {msg})", this.LoggingContext);
        ...
        private void Retry()
        {
            HTTPManager.Logger.Information("EventSource", $"Retry({this.State})", this.LoggingContext);
...
#region HTTP Request Implementation
        ...
        private void OnRequestFinished(HTTPRequest req, HTTPResponse resp)
        {
            ...
            switch (req.State)
            {
                ...
                case HTTPRequestStates.Finished:
                    ...
                    {
                        // https://github.com/Benedicht/BestHTTP-Issues/issues/168
                        // https://html.spec.whatwg.org/multipage/iana.html#text/event-stream
                        // Content-Type's value must be "text/event-stream", but it might have an optional charset parameter
                        var contentTypeValue = resp.GetFirstHeaderValue("content-type");

                        if (contentTypeValue == null)
                        {
                            canRetry = false;
                            reason = "No Content-Type header found!";
                            break;
                        }

                        HeaderParser contentType = new HeaderParser(contentTypeValue);
                        bool eventStream = contentType?.Values?.FirstOrDefault()?.Key == "text/event-stream";

                        if (!eventStream)
                        {
                            reason = $"No Content-Type header with value 'text/event-stream' present. Got '{contentTypeValue}'";
                            canRetry = false;
                            break;
                        }
        ...
        private bool OnData(HTTPRequest request, HTTPResponse response, byte[] dataFragment, int dataFragmentLength)
        {
            if (HTTPManager.Logger.Level == Loglevels.All)
                HTTPManager.Logger.Information("EventSource", $"OnData({dataFragment.AsBuffer(dataFragmentLength)})", this.LoggingContext);
...
#region Data Parsing
        ...
        {
            if (HTTPManager.Logger.Level == Loglevels.All)
                HTTPManager.Logger.Information("EventSource", $"FeedData({buffer.AsBuffer(count)})", this.LoggingContext);
        ...
        bool ParseLine(byte[] buffer, int count)
        {
            ...
            if (count == 0)
            {
                ...
                {
                    if (HTTPManager.Logger.Level == Loglevels.All)
                        HTTPManager.Logger.Information("EventSource", $"ParseLine - event dispatch ({this.State}, {CurrentMessage})", this.LoggingContext);
            ...
            if (buffer[0] == 0x3A)
            {
                var msg = new Message() { IsComment = true, Data = Encoding.UTF8.GetString(buffer, 1, count - 1) };

                if (HTTPManager.Logger.Level == Loglevels.All)
                    HTTPManager.Logger.Information("EventSource", $"ParseLine - comment (':') found ({this.State}, {msg})", this.LoggingContext);

                this.CompletedMessages.Enqueue(msg);
                return true;
            }
...
#region EventStreamResponse Event Handlers
        ...
        {
            if (HTTPManager.Logger.Level == Loglevels.All)
                HTTPManager.Logger.Information("EventSource", $"OnMessageReceived({this.State}, {message})", this.LoggingContext);
